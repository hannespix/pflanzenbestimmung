...
/* ---------- Bilder-Quiz: Foto erkennen (opt-in, braucht Internet) ----------
   ...
   nutzbar. */
const LS_PHOTOS = LS_PREFIX+"photos3";                // gemerkte Bild-URLs (spart API-Abrufe; »3« = strengere Auswahl, gemerkte Fehltreffer wie Buchscans verfallen)
...
function usablePhoto(file){
  if(!file) return true;
  // Dokumentformate ganz raus: Das »Vorschaubild« eines PDF/DjVu ist die erste
  // Seite – bei digitalisierten Büchern also Einband oder Titelblatt (der
  // Garten-Dill bekam so den marmorierten Deckel von Hills »Hortus Kewensis«).
  if(/\.(svg|pdf|djvu|tiff?)$/i.test(file)) return false;
  if(/\(ia[ _][a-z0-9]/i.test(file)) return false;    // Internet-Archive-Digitalisat, z. B. »… (IA b30512888).pdf«
...
function photoSteps(card){                            // Suchwege, vom genauesten zum gröbsten
  ...
  // Deutsche Namen wie beim Info-Modal in Artikeltitel-Varianten auflösen
  // (»Garten-Dill« → auch »Gartendill«, »Kohl- / Steck-Rübe« → »Kohlrübe«) –
  // die Listen-Schreibweise allein trifft den Wikipedia-Titel oft nicht.
  const deT = deArticleTitles(card).slice(0, 3);
  ...
  if(isCultivarName(card.a)){
    ...
    deT.forEach(t=>steps.push({k:"wp", q:t}));
    if(de) steps.push({k:"cm", q:`"${de}"`});
...
/* needName: Bei VOLLTEXT-Suchen (ohne incategory:) matcht die Anfrage auch
   Datei-BESCHREIBUNGEN – und »Anethum graveolens hortorum« findet dann seitenweise
   digitalisierte Kräuterbücher, in deren Text die Wörter vorkommen. Solche Treffer
   sind nur brauchbar, wenn der DATEINAME die Pflanze selbst nennt (Score ≥ 1).
   Kategorie-Treffer dürfen dagegen namenlos sein – die Kategorie ist das Signal. */
function pickCommons(d, card, needName){              // bester Treffer der Commons-Suche
...
async function wikiPhoto(card){                        // → {thumb,title,file,url,src} | null
  ...
  for(const st of photoSteps(card)){
    try{
      if(st.k==="cm"){
        const hit = pickCommons(await commonsQuery(st.q), card, st.q.indexOf("incategory:") < 0);
        answered = true;
        if(hit) return hit;
      }else{
